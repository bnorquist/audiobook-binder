...
This installs the `audiobook` CLI command in the project's virtual environment. Run it with `uv run audiobook` or activate the venv first.
...
```bash
audiobook convert ./my-mp3s/ -o "My Book.m4b" --title "My Book" --author "Jane Author"
```
...
audiobook convert ./my-mp3s/ -o "My Book.m4b"
```

### Preview without converting

```bash
audiobook convert ./my-mp3s/ --dry-run
```
...
```
audiobook init INPUT_PATH [-o OUTPUT]
```
...
The manifest is a YAML file that controls file ordering, chapter names, and book metadata. Generated by `audiobook init`, meant to be edited by hand.

```yaml
# Generated by: audiobook init ./my-mp3s
# Edit this file to customize your audiobook, then run:
#   audiobook convert ./my-mp3s
